...
module.exports = {
  ...
  returnSomeBooks: function(genre, callback){
    Book_Authors().where('genre', genre).from('book_authors').leftJoin('books', 'book_authors.book_id', 'books.id').leftJoin('authors', 'authors.id', 'book_authors.author_id').then(function(allinfo){
