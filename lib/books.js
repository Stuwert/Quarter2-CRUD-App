...
module.exports = {
  ...
  returnSomeBooks: function(query, parameter, callback){
    Book_Authors().where(parameter, query).from('book_authors').leftJoin('books', 'book_authors.book_id', 'books.id').leftJoin('authors', 'authors.id', 'book_authors.author_id').then(function(allinfo){
      callback(newBookArray(allinfo));
    })
  },
