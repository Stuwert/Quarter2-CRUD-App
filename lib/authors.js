...
module.exports = {
  ...
  someAuthors: function(queryArray, callback){
    Authors().from('authors').leftJoin('book_authors', 'book_authors.author_id', 'authors.id')
      .leftJoin('books', 'book_authors.book_id', 'books.id')
      .then(function(authors){
        callback(queryFilter(queryArray, newAuthorArray(authors)))
    })
  },
  ...
  deleteAuthor: function(id, callback){
    Authors().where('id', id).del().then(function(){
      Book_Authors().where('author_id', id).del().then(function(){
        callback();
      })
    })
  }
...
function queryFilter(queryArray, authorArray){
  return authorArray.filter(function(item){
    var bool = false;
    queryArray.forEach(function(queryitem){
      if (item.name.toLowerCase().includes(queryitem.toLowerCase())){
        bool = true;
      }
    })
    return bool;
